...
## V0.7 Commander Protocol 指挥官判断协议

V0.7 固化“AI 指挥官 / 战略总控”的回答规则，不新增 Dashboard、不部署、不接数据库、不自动派发工具。

### Commander Protocol

- 新增 `context/commander-protocol.md`，明确：先判断方向、降低复杂度、保护注意力，再决定是否需要执行提示词。
- 协议包含默认分类、Goal 使用规则、旧项目规则、新机会判断维度、执行判断块、项目体检、开工包、工具选择、复杂化检查和敏感信息脱敏规则。
- Ask Mode 会把 Commander Protocol 作为回答规则层注入 LLM prompt；它优先于普通项目上下文，但不覆盖安全脱敏规则。

### Ask 回答变化

- 遇到任务提示词、项目想法、功能需求或执行请求时，优先做执行判断。
- 遇到 Dashboard、数据库、登录、支付、部署、自动派发多个智能体、验证前完整 MVP、同时做多个方向等复杂化信号时，倾向于回答“这件事可以先收缩”，并给出今天最小动作。
- 搜索结果仍只是补充上下文，不等于结论；默认不联网原则不变。
- iPortfolio / 个人网站、小Chan、节律 App 继续作为学习材料，不因趋势默认建议更新、重构、部署或扩展旧项目。

### 任务提示词模板

- “复制为 Codex 任务”模板升级为工程执行结构：项目路径、当前状态、目标、不要做、必须完成、执行要求、测试要求、真实验证、Git 要求、最终汇报。
- “复制为 Claude Code 任务”模板升级为前端 / UI / 交互结构：项目路径、当前状态、目标、不要做、允许修改范围、必须完成、真实网页验证、测试要求、最终汇报。
- 任务提示词仍只复制到剪贴板，不自动调用 Codex / Claude Code / OpenDesign / MiniMax。

### 保持不变

- 默认不联网、按需搜索、Goal 匹配度、今日优先级、固定视口工作台、loading 动画、Bocha / Tavily provider、LLM 配置和 `sk-*` 全链路脱敏均不变。
- `.env`、`opportunity-pool.json`、`data/opportunities/backups`、用户生成数据不提交。
