...
当前状态：
以下是 EricChan·战略OS 生成的开工包。请基于它执行；如果信息不足，先做现状检查，不要凭空扩大需求。
${goalSection}

目标：
完成本次开工包指向的最小必要工程任务，并保持边界清晰。
...
当前状态：
以下是 EricChan·战略OS 生成的开工包。当前页面 / 功能状态以项目现状为准，先检查再修改。

目标：
完成本次开工包指向的最小必要页面或交互改善。
...
允许修改范围：
只改与开工包目标直接相关的前端、样式、脚本和测试；不要做无关重构。

必须完成的事：
1. 明确本次 UI / 交互目标
2. 保持已有视觉点、固定视口和默认不联网不回归
3. 补最小必要交互测试
