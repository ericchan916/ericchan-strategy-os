...
test("bocha real data wrapper response is normalized", async () => {
  ...
  assert.equal(result.results.length, 2);
  const datedResult = result.results.find((item) => item.url === "https://example.cn/agent");
  const undatedResult = result.results.find((item) => item.url === "https://agent.example/path");
  assert.equal(datedResult.title, "真实结构结果");
  assert.equal(datedResult.snippet, "真实结构摘要优先");
  assert.equal(datedResult.source, "ExampleCN");
  assert.equal(datedResult.publishedAt, "2026-07-04T00:00:00+08:00");
  assert.ok(datedResult.quality.overallScore > 0);
  assert.equal(undatedResult.source, "agent.example");
